...
Binding.prototype.existsSync = function(path) {
  return !!this._system.getItem(path);
}


Binding.prototype.writeFileUtf8 = function(file, content, flags, mode) {
  let destFlags =
        constants.O_WRONLY | constants.O_CREAT | constants.O_TRUNC;

  const dest = deBuffer(file);

  const destFd = this.open(dest, destFlags, mode);

  try {
    this.writeBuffer(destFd, content.toString('utf-8'), 0, content.length, 0);
  } finally {
    this.close(destFd);
  }
}
